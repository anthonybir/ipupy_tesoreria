...
const createConnection = () => {
  ...
  if (!pool) {
    ...
    pool = new Pool({
      ...
      // Connection settings - Vercel optimized
      connectionTimeoutMillis: 3000,  // Reduced from 8000 for Vercel's 10s limit
      idle_timeout: 20000,           // Close idle connections after 20s
      max: process.env.VERCEL ? 1 : 10,  // Single connection for serverless

      // Health settings
      allowExitOnIdle: false,        // Keep pool alive between invocations
...
const execute = async (statement, params = [], retries = 2) => {  // Reduced from 3 to 2 for Vercel
    ...
    try {
      ...
      const client = await Promise.race([
        ...
        new Promise((_, reject) =>
          setTimeout(() => reject(new Error('Pool connect timeout')), 3000)  // Reduced from 8000
        )
      ]);
      ...
      // Minimal delay for Vercel's 10s limit
      const delay = 500 + (attempt * 200);  // 700ms, 900ms
