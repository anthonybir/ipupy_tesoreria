...
const getConnectionString = () => {
  ...
  let connectionString = (process.env.SUPABASE_DB_URL || process.env.DATABASE_URL || '').trim();
  if (!connectionString) {
  ...
  // For Supabase connections, handle different pooler types
  if (connectionString.includes('supabase.co:6543')) {
    ...
    // Session pooler - already IPv4 compatible
    const url = new URL(connectionString);
...
const createConnection = () => {
  ...
  if (!pool) {
    ...
    pool = new Pool({
      ...
      max: process.env.VERCEL ? 3 : 10,  // Smaller pool for serverless

      // Health settings
      allowExitOnIdle: true,          // Allow process to exit when idle

      // Force IPv4 for Vercel compatibility
      // This is critical as Vercel uses IPv4 and Supabase needs proper DNS resolution
...
const execute = async (statement, params = [], retries = 3) => {
  for (let attempt = 1; attempt <= retries; attempt++) {
    try {
      ...
      const delay = Math.min(baseDelay + jitter, 3000); // Cap at 3s for faster recovery
