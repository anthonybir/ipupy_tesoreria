...
async function handleGet(req, res, decoded) {
  try {
    ...
    let paramIndex = 1;
    if (church_id) {
      sql += ` AND er.church_id = $${paramIndex++}`;
    ...
    if (report_id) {
      sql += ` AND er.report_id = $${paramIndex++}`;
      params.push(report_id);
    }

    if (fecha_inicio) {
      sql += ` AND er.fecha_comprobante >= $${paramIndex++}`;
    ...
    if (tipo_salida) {
      sql += ` AND er.tipo_salida = $${paramIndex++}`;
      params.push(tipo_salida);
    }

    if (proveedor) {
      sql += ` AND er.proveedor LIKE $${paramIndex++}`;
    ...
    if (church_id) {
      countSql += ` AND er.church_id = $${countParamIndex++}`;
    ...
    if (report_id) {
      countSql += ` AND er.report_id = $${countParamIndex++}`;
      countParams.push(report_id);
    }

    if (fecha_inicio) {
      countSql += ` AND er.fecha_comprobante >= $${countParamIndex++}`;
    ...
    if (tipo_salida) {
      countSql += ` AND er.tipo_salida = $${countParamIndex++}`;
      countParams.push(tipo_salida);
    }

    if (proveedor) {
      countSql += ` AND er.proveedor LIKE $${countParamIndex++}`;
...
async function handlePut(req, res, decoded) {
  try {
    ...
    const existingExpense = await execute(
      'SELECT church_id FROM expense_records WHERE id = $1',
      [expenseId]
    );
    ...
    await execute(`
      UPDATE expense_records SET
        fecha_comprobante = $1, numero_comprobante = $2, ruc_ci_proveedor = $3,
        proveedor = $4, concepto = $5, tipo_salida = $6,
        monto_exenta = $7, monto_gravada_10 = $8, iva_10 = $9,
        monto_gravada_5 = $10, iva_5 = $11, total_factura = $12,
        es_factura_legal = $13, es_honorario_pastoral = $14, observaciones = $15
      WHERE id = $16
    `, [
...
async function handleDelete(req, res, decoded) {
  try {
    ...
    const result = await execute('DELETE FROM expense_records WHERE id = $1', [expenseId]);
