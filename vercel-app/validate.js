    ...
    if (fs.existsSync(filePath)) {
        try {
            ...
            if (content.includes('export default async function handler')) {
                ...
                errors.push(`${file} debe exportar una función handler por defecto`);
            }
...
try {
    ...
    if (dbContent.includes('const { Pool } = require(\'pg\')')) {
        console.log('  ✅ PostgreSQL configurado');
    } else {
        console.log('  ❌ PostgreSQL no configurado correctamente');
        errors.push('lib/db.js debe usar PostgreSQL (pg)');
    }

    if (dbContent.includes('process.env.DATABASE_URL')) {
        console.log('  ✅ Variable de entorno DATABASE_URL referenciada');
    } else {
        console.log('  ❌ Variable de entorno DATABASE_URL no referenciada');
        errors.push('lib/db.js debe usar process.env.DATABASE_URL');
    }
...
try {
    const appContent = fs.readFileSync(path.join(__dirname, 'public', 'app.html'), 'utf8');
...
try {
    ...
    const requiredVars = ['DATABASE_URL', 'JWT_SECRET', 'ADMIN_EMAIL', 'NODE_ENV'];
